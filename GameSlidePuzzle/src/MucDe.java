...
public class MucDe{
//	Tạo đối tượng
	JFrame p = new JFrame("Game Slide Puzzle (Mức dễ)");
...
//		Ẩn các vùng chơi chính, vùng so sánh và vùng trạng thái
...
//		Xử lí các sự kiện cho các nút lệnh trên menubar
...
//		Cài đặt vùng chơi chính
		...
		vungchoichinh.setBounds(0, 0, 300, 400);
		int x=0,y=0;
		for (int i=1;i<=12;i++){
			Manh[i] = new JButton();
...
//		Gắn sự kiện nút bấm cho các mảnh ghép ở vùng chơi chính
	...
	protected void CompareImage(){
		anhsosanh.setBounds(320, 0, 180, 240);
		p.add(anhsosanh);
	}
	...
	protected void OriginImage(){
		this.vungchoichinh.setVisible(false);
		this.anhgoc.setVisible(true);
		this.anhgoc.setBounds(0, 0, 300, 400);
		p.add(anhgoc);
		this.anhgoc.setIcon(new ImageIcon("images\\de\\"+this.hinhmau+"\\goc.jpg"));
	}
	
	protected void status(){
		p.add(trangthai);
		trangthai.setBounds(320, 250, 180, 140);
...
//	gắn hình ảnh vào JLabel và JButton
	protected void anhxep(){
		for (int i=1;i<=11;i++)
			Manh[i].setIcon(new ImageIcon("images\\de\\"+this.hinhmau+"\\("+i+").jpg"));
		Manh[12].setIcon(null);
	}
...
//		Cài hình
		this.anhxep();
		anhsosanh.setIcon(new ImageIcon("images\\de\\"+this.hinhmau+"\\sosanh.jpg"));
...
//	Xử lý đối với các mảnh
	...
	private void manh5ActionPerformed(ActionEvent evt){
		if (vitri == 8 || vitri == 6 || vitri == 4 || vitri == 2){
	...
	private void manh8ActionPerformed(ActionEvent evt){
		if (vitri == 11 || vitri == 9 || vitri == 7 || vitri == 5){
...
//	Gắn hình ảnh
	private void ganhinhanhmau(){
		for (int i=1;i<=8;i++)
			mau[i].setIcon(new ImageIcon("images\\de\\Mau"+i+".jpg"));
	}
...
//	Xử lý sự kiện nhấn nút
	...
	JButton wchoitiep = new JButton("Chơi tiếp mức vừa");
